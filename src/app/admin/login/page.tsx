...
export default function AdminLoginPage() {
  ...
  const handleLoginSubmit = async (e?: React.FormEvent) => {
    ...
    if (!adminEmail.trim()) {
      setErrorMsg("Please enter your Secretariat Email");
      return;
    }
    ...
    if (success) {
      ...
      setErrorMsg("Authentication failed. Please verify administrator passcode.");
      setIsLoading(false);
    }
  };
  ...
  const handle1ClickDemo = async () => {
    setErrorMsg("");
    setIsLoading(true);
    setAdminEmail("sunita.murmu@mota.gov.in");
    setPassword("123456");
      ...
      <div className="relative z-10 max-w-md w-full my-8 bg-white/90 sm:bg-white/95 backdrop-blur-xl rounded-[36px] p-7 sm:p-9 shadow-2xl border border-white/80 transition-all">
        ...
        <div className="flex flex-col items-center text-center mb-4">
          ...
          <div className="text-xs font-black tracking-wider uppercase text-amber-900/80">
            Executive Secretariat • MoTA India
          </div>
        </div>
        ...
        <div className="bg-[#EBF6ED] border border-[#C2E7C8] rounded-2xl p-4 text-center space-y-2 mb-6">
          <div className="flex items-center justify-center gap-1.5 text-xs font-black text-[#1C3C28]">
            <span>💡</span>
            <span>Quick Demo Access (Smt. Sunita Murmu IAS)</span>
          </div>
          <div className="text-[11px] text-[#225732] font-semibold">
            Email: <strong className="font-mono text-[#1C3C28]">sunita.murmu@mota.gov.in</strong> | PIN: <strong className="font-mono text-[#1C3C28]">123456</strong>
          </div>
          <button
            type="button"
        ...
        <form onSubmit={handleLoginSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="e.g. sunita.murmu@mota.gov.in"
          ...
          <div>
            <label className="block text-xs font-bold text-[#1C3C28] mb-1.5 pl-1">
              Administrator Passcode / 2FA Token
            </label>
          ...
          >
            {isLoading ? (
              <span className="flex items-center gap-2">
                ...
                <span>Opening Command Center...</span>
              </span>
            ) : (
              <span>Log in</span>
            )}
        ...
        <div className="text-center mt-5">
          ...
          >
            Policy & Gazette Desk • Government of India
          </Link>
        </div>
      </div>
